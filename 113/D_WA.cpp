...
struct Matrix
{
	...
	{
		Matrix res(n);
		Strassen_class<double> stra;
		stra.Strassen(n, a, other.a, res.a);
		
		return res;
	}*/
	
	Matrix calPow(int k) const
	{
		Matrix res(n);
		res.unit();
		Matrix cur(*this);
...
int main()
{
	...
	int n, m, i, u, v, a, b, nextState, initState;
	cin >> n >> m >> a >> b;
	a--;
	b--;
	for (i = 0; i < m; i++)
	...
	// to reduce stateNum, let u >= v
	
	int stateNum = n * (n - 1) / 2 + n;
	Matrix mat(stateNum);
	double tmp, pu, pv;
	for (i = 0; i < stateNum; i++)
	{
		...
		pu = (1.0 - p[u]) / graph[u].size();
		pv = (1.0 - p[v]) / graph[v].size();
		
		// u leave, v stay
		for (auto nextU : graph[u])
		{
			nextState = getState(nextU, v);
			mat[i][nextState] += pu * p[v];
		}
	...
	for (u = 0; u < n; u++)
	{
		int curState = getState(u, u);
		for (i = 0; i < stateNum; i++)
		{
			mat[curState][i] = 0;
		}
		mat[curState][curState] = 1;
	}
	
	//mat.print();
	Matrix tranMat = mat.calPow(100000);
	//Matrix tranMat(mat.power((int)1e6));
	initState = getState(a, b);
	for (i = 0; i < stateNum; i++)
	{
		getUV(i, u, v);
		if (u == v)
		{
			ans[u] = tranMat[initState][i];
		}
	}
