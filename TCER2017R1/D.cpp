...
public:
	...
	int dinic(int vNum)
	{
		...
		{
			if (edge[i].v > 0 && edge[i].v <= vNum && edge[i].u != t && !edge[i].cap)
			{
				ans[edge[i].v] = edge[i].u - vNum;
			}
		}
...
int main()
{
	...
	for (i = 1; i <= n; i++)
	{
		...
		if (pos != string::npos)
		{
			...
			maxFlowObj.addEdge(n + idx, i, 1);
			
			idx = str2Num(cur.substr(pos + 1, cur.size() - pos - 1));
			maxFlowObj.addEdge(n + idx, i, 1);
			
			maxFlowObj.addEdge(i, m, 1);
		}
		else
		{
			c++;
	...
	if (ok)
	{
		for (i = 1; i <= 6; i++)
		{
			if (num[i])
			{
				maxFlowObj.addEdge(0, n + i, num[i]);
			}
